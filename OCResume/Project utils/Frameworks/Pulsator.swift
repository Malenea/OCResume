...
open class Pulsator: CAReplicatorLayer, CAAnimationDelegate {
    ...
    fileprivate func setupAnimationGroup(isSolid: Bool) {
        ...
        opacityAnimation.values = isSolid ? [alpha] : [alpha, alpha * 0.5, 0.0]
    ...
    /// Start the animation.
    @objc open func start(isSolid: Bool = false) {
        setupPulse()
        setupAnimationGroup(isSolid: isSolid)
        pulse.add(animationGroup, forKey: kPulsatorAnimationKey)
    }
